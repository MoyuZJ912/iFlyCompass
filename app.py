...
class NovelReadingProgress(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    novel_filename = db.Column(db.String(255), nullable=False)  # 小说文件名
    last_chapter_index = db.Column(db.Integer, default=0)  # 最后阅读的章节索引
    last_read_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))  # 最后阅读时间
    
    # 联合唯一约束：一个用户对每本小说只能有一个阅读进度
...
@app.route('/api/novels/<novel_name>/progress', methods=['POST'])
@login_required
...
@app.route('/api/novels/<novel_name>/progress', methods=['GET'])
@login_required
